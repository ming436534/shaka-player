 ...
 *   different Period.
 * @param {function()} onFreezing Called when freeze
 *
 * @constructor
 * @struct
...
shaka.media.PlayheadObserver = function(
    video, manifest, config, onBuffering, onEvent, onChangePeriod, onFreezing) {
...
shaka.media.PlayheadObserver.prototype.onWatchdogTimer_ = function() {
  ...
  if (!this.buffering_) {
    ...
    if (!atEnd && bufferedAhead < threshold) {
      ...
      if (this.video_.readyState === 1 && this.video_.playbackRate === 1) {
        if (this.prevTime === this.video_.currentTime) {
  ...
  this.prevTime = this.video_.currentTime;
