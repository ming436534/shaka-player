...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    this.detectAndCorrectTimeSync_(manifest);
...
/**
 * Look for common time sync issues in the timeline and media segments.
 * If found, warn about the issue and attempt to correct it.
 *
 * @param {shakaExtern.Manifest} manifest
 * @private
 */
shaka.Player.prototype.detectAndCorrectTimeSync_ = function(manifest) {
  var timeline = manifest.presentationTimeline;
  if (!timeline.isLive()) {
    // Time sync doesn't exist for VOD.
    return;
  }

  var lastPeriod = manifest.periods[manifest.periods.length - 1];
  if (!lastPeriod || !lastPeriod.variants.length) {
    shaka.log.debug('Unable to check media segment references for time sync.');
    return;
  }

  var anyVariant = lastPeriod.variants[0];
  var anyStream = anyVariant.video || anyVariant.audio;
  if (!anyStream) {
    shaka.log.debug('Unable to check media segment references for time sync.');
    return;
  }

  var segments = anyStream.mediaSegmentReferences;
  if (!segments || !segments.length) {
    shaka.log.debug('Unable to check media segment references for time sync.');
    return;
  }

  shaka.log.debug('Segment size is', timeline.getMaxSegmentDuration());

  var lastSegment = segments[segments.length - 1];
  var offset = lastSegment.endTime - timeline.getSegmentAvailabilityEnd();
  if (Math.abs(offset) <= timeline.getMaxSegmentDuration()) {
    shaka.log.debug('Segments are close enough to the timeline.', offset);
    shaka.log.debug('No timeline correction.');
    return;
  }

  shaka.log.warning('Apparent timeline desync is', offset, 'seconds');
  shaka.log.warning('Attempting to correct the timeline.');
  timeline.setClockOffset(offset * 1000);  // milliseconds
};
...
shaka.Player.prototype.createPlayheadObserver = function() {
  ...
  return new shaka.media.PlayheadObserver(
      ...
      this.onChangePeriod_.bind(this), this.onFreezing_.bind(this));
};
...
shaka.Player.prototype.getStats = function() {
  ...
  return {
    ...
    estimatedBandwidth: this.abrManager_.getBandwidthEstimate(),
 ...
 * @param {!Object.<string, !shakaExtern.Stream>} streams
 ...
 * @private
 */
shaka.Player.prototype.onFreezing_ = function() {
  if (this.playhead_) {
    var currentTime = this.video_.currentTime;
    var trialTime = currentTime + 2.5;
    this.playhead_.movePlayhead_(currentTime, trialTime);
  }
};
